...
namespace KDS_Pizzeria
{
	public partial class FormKDS : Form
	{
		...
		private void TimerKDS_Tick(object sender, EventArgs e)
		{
			...
			var pedidosASerRemovidos = new List<int>();

			foreach (var pedido in PedidosExibidos)
			{
				if (pedidos.Any(p => p.ID == pedido))
				{
					continue;
				}
				else
				{
					pedidosASerRemovidos.Add(pedido);

					this.panelKDS.Controls.RemoveByKey($"PanelPedido{pedido}");
				}
			}

			PedidosExibidos.RemoveAll(p => pedidosASerRemovidos.Contains(p));
